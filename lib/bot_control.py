...
def move_bot(bot_driver, ip, y_speed, x_speed, move_duration, sequenceId=0, bot=None):
...
def move_bot_direct(ip, y_speed, x_speed, move_duration, sequenceId):
    now = datetime.now()
    body = '{"angle":%.4f,"velocity":%.4f,"duration":%i,"sequenceId":%i}' % (
        y_speed, x_speed, move_duration, sequenceId)
    r = requests.put('http://%s/api/set_manual_control' %
                     ip, data=body, headers=headers)
    # print(r)
    # self.sequenceId+=1
